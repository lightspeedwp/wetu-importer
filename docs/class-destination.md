...
- **Side effects:** Delegates to `$this->set_country()` (base class), which may create a draft `destination` post from the country fetch result if one doesn't already exist, storing its WETU ID via the `lsx_wetu_id` post meta — this is the source of the "check the draft list for countries" note shown in `display_page()`.
