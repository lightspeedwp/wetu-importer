...
- **`classes/class-wetu-automation.php` → `pins_sync_action()`** calls the Wetu Pins API using a hardcoded API key literal (`[redacted]`) baked into the URL instead of `get_api_key()` — likely a leftover placeholder/debug value that should use the configured key. Rotate this credential if it was ever a real/active key.
- **`LSX_WETU_Importer_Tours::check_if_image_is_used( $v )`** ([class-tours.md](class-tours.md)) is passed a full image array (`url_fragment`, `label`, `description`, etc.) but queries `wp_postmeta` with that array coerced into a `%s` scalar placeholder, compared against the `lsx_wetu_id` meta key (which stores the tour/pin's own Wetu entity ID, not an image identifier). The array-to-string coercion means the comparison will not match real data, so the check is effectively a no-op and never prevents reusing the same image across tours.
...
- **`LSX_WETU_Importer_Banner_Integration::sync_new_banner()`** ([class-banner-integration.md](class-banner-integration.md)) is registered on both `wp_ajax_lsx_import_sync_banners` and `wp_ajax_nopriv_lsx_import_sync_banners`, and only checks `check_ajax_referer()` — no `current_user_can()` capability check, and no validation that the supplied `post_id` is an `accommodation` post the requester should be able to modify. This allows unauthenticated requests to trigger external HTTP fetches and attachment creation for an arbitrary post ID.
- **`LSX_WETU_Importer_Settings::save_options()`** ([class-settings.md](class-settings.md)) verifies the settings-form nonce but never calls `current_user_can()` before `update_option( 'lsx_wetu_importer_settings', ... )` — nonce validation alone is not an authorization check.
...
- **`LSX_WETU_Importer_Post_Columns::register_sortable_columns()`** is not wired to any hook in `__construct()` (unlike `register_tour_columns()`/`output_tour_ref_column()`/`tour_search_by_wetu_ref()`), so it appears to be dead/unused code unless attached elsewhere. It also maps `wetu_ref` to sort key `price_per_month` instead of `lsx_wetu_ref` — if it is ever wired up, the Ref column would sort by monthly price rather than by Wetu reference.
