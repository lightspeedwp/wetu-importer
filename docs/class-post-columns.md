...
Adds `wetu_ref` to a sortable-columns array, but maps it to sort key `price_per_month` instead of `lsx_wetu_ref` — if this method is ever wired up to the `manage_edit-tour_sortable_columns` filter, sorting by the Ref column would order by monthly price rather than the Wetu reference.
- **Note:** not wired up to any hook in `__construct()`, so it currently appears unused/dead code — see [Known Issues](known-issues.md), which also tracks the incorrect `price_per_month` sort key mapping.
