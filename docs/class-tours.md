...
- **Side effects:**
  ...
  - Calls `set_itinerary_day()` once per leg iteration, each time passing the entire `$days` array accumulated so far (`$days` is never reset between legs). Because `save_custom_field()` writes `itinerary` as a unique meta key, each call replaces the previous value rather than adding a new row, so the post ends up with a single `itinerary` meta value containing the complete day list — there is no cumulative/duplicate-row buildup across legs, despite the repeated writes.
...
Intended to check whether a given Wetu image is already associated with any existing `tour` post, to avoid reusing the same destination image across multiple tours. `$v` is passed the full image array (`url_fragment`, `label`, `description`, etc.) from `attach_image()`'s caller, but the query compares it as a scalar `%s` against the `lsx_wetu_id` meta key — which stores the tour/pin's own Wetu entity ID, not an image identifier. In practice the array is coerced to the string `"Array"` before the query runs, so the `WHERE meta_value = %s` comparison will not match real `lsx_wetu_id` values; this check is effectively a no-op (always reports the image as unused) rather than a working image-deduplication check.
