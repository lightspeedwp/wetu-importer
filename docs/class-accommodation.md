...
- **Side effects:** Indirect, via `set_destination()` (base class). `set_destination()` calls `set_country()` and, when no matching destination post exists, `wp_insert_post()` — creating a new `destination` post in draft status.
...
- **Side effects:** Post meta `rating_type` and `rating`, written via `save_custom_field()`, which calls `update_post_meta()` when the meta key already exists or `add_post_meta()` when it does not; the third argument passed is the previous value used for the update comparison, not an overwrite flag.
