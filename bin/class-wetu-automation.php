...
class WETU_Automation {
			...
			foreach ( $tours as $tour ) {
				...
				//if ( ! empty( $wetu_tours ) && isset( $wetu_tours[ $wetu_id ] ) ) {
					...
					if ( false !== $tour_date && '' !== $tour_date ) {
						...
						print_r($tour_date);
						print_r('-');
						print_r($wetu_tours[ $wetu_id ]['last_modified']);
						print_r('</pre>');*/
	...
	public function tour_sync_action( $tour_id, $overrides = array() ) {
		...
		$importable = apply_filters( 'wetu_automation_tour_flags', $flags );

		if ( false !== $wetu_id && '' !== $wetu_id ) {

			// First lets check the modified dat.
			$tour_date = get_post_meta( $tour_id, 'lsx_wetu_modified_date', true );
			$date_data = wp_remote_get( 'https://wetu.com/API/Itinerary/V8/Get?id=' . $wetu_id );
			if ( false !== $tour_date && '' !== $tour_date ) {
				$tour_mod = strtotime( $tour_date );
				$wetu_mod = strtotime( $jdata['last_modified'] );

				if ( $wetu_mod <= $tour_mod ) {
				}
			}

			// Then lets import the info.
			...
			if ( null !== $response ) {
				...
				update_post_meta( $tour_id, 'lsx_wetu_sync_date', strtotime( 'now' ) );
			}
		}
		return $return;
	}
